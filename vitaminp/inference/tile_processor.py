...
class TileDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, Dict]:
        ...
        # PADDING APPROACH: Pad to target size
        target_size = 1024
        h, w = tile_img.shape[:2]
        
        if h != target_size or w != target_size:
            import cv2
            # Calculate padding (bottom and right padding only)
            pad_h = target_size - h
            pad_w = target_size - w
            
            # Pad with zeros (black) or reflect
            tile_img = cv2.copyMakeBorder(
                tile_img, 
                0, pad_h,  # top, bottom
                0, pad_w,  # left, right
                cv2.BORDER_CONSTANT,  # or cv2.BORDER_REFLECT for better results
                value=[0, 0, 0]  # black padding
            )
            
            # Store original size in metadata for cropping predictions later
            tile_meta.original_height = h
            tile_meta.original_width = w
        else:
            tile_meta.original_height = h
            tile_meta.original_width = w
...
class TileProcessor:
    ...
    def process_wsi(
        ...
        Returns:
            Tuple[List[torch.Tensor], List[Dict]]:
                - List of predictions for each tile
        ...
        with torch.no_grad():
            for batch_imgs, batch_meta in tqdm(dataloader, desc="Processing tiles"):
                ...
                if isinstance(predictions, dict):
                    # Model returns dictionary (e.g., with features)
                    if return_features:
                        pred_output = predictions
                    else:
                        # Extract main predictions
                        pred_output = predictions.get('logits', predictions.get('output', predictions))
                ...
                # Store predictions (keep on CPU to save GPU memory)
                if isinstance(pred_output, dict):
                    # Store dictionary of tensors
                    pred_dict = {k: v.cpu() for k, v in pred_output.items()}
                    all_predictions.append(pred_dict)
                else:
                    # Store tensor
                    all_predictions.append(pred_output.cpu())
    ...
    def _collate_fn(
        ...
        Args:
            batch (List[Tuple[torch.Tensor, Dict]]): List of (image, metadata) tuples
