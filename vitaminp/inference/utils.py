...
def load_wsi_metadata(
    ...
    Returns:
        Tuple[Dict, float]: 
            - Dictionary with 'mpp' and 'magnification' keys
            - Target MPP to use for inference

    Raises:
        RuntimeError: If OpenSlide is not available
        ValueError: If MPP or magnification cannot be determined
    """
    if not OPENSLIDE_AVAILABLE:
        raise RuntimeError("OpenSlide is required. Install with: pip install openslide-python")
    ...
    if wsi_properties is not None and "slide_mpp" in wsi_properties:
        ...
        try:
            ...
            match = pattern.search(slide.properties.get("openslide.comment", ""))
            if match:
