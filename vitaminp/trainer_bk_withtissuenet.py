...
class VitaminPTrainer:
    ...
    def validate(self):
        ...
        # ---------------------------------------------------------------------
        # VALIDATION STRATEGY: FLEX (CORRECTED FOR TISSUENET)
        # ---------------------------------------------------------------------
        elif self.model_type == 'Flex':
            val_loss_accum = 0
            ...
            for batch in self.val_loader:
                ...
                if len(valid_he_indices) > 0:
                    ...
                    loss_he = (
                        self.seg_criterion(outputs_he['he_nuclei_seg'], n_mask_sub) +
                        2.0 * self.hv_criterion(outputs_he['he_nuclei_hv'], n_hv_sub, n_mask_sub, self.device) +
                        self.seg_criterion(outputs_he['he_cell_seg'], c_mask_sub) +
                        2.0 * self.hv_criterion(outputs_he['he_cell_hv'], c_hv_sub, c_mask_sub, self.device)
                    ) / 2.0
                    
                    val_loss_accum += loss_he.item()
                    
                    # Accumulate weighted metrics
                    count = len(valid_he_indices)
                    metrics['he_nuclei_dice'] += compute_dice((outputs_he['he_nuclei_seg'] > 0.5).float(), n_mask_sub).item() * count
                    metrics['he_cell_dice'] += compute_dice((outputs_he['he_cell_seg'] > 0.5).float(), c_mask_sub).item() * count
                    metrics['he_nuclei_hv_std'] += outputs_he['he_nuclei_hv'].std().item() * count
                    metrics['he_cell_hv_std'] += outputs_he['he_cell_hv'].std().item() * count
                    
                    val_he_count += count

                # --- 2. MIF VALIDATION PASS (All Samples) ---
                # All samples including TissueNet have MIF
                mif_img = batch['mif_image'].to(self.device)
                mif_img = prepare_mif_input(mif_img)
                mif_img = self.preprocessor.percentile_normalize(mif_img)
                
                outputs_mif = self.model(mif_img)
                
                loss_mif = (
                    self.seg_criterion(outputs_mif['mif_nuclei_seg'], nuclei_mask) +
                    2.0 * self.hv_criterion(outputs_mif['mif_nuclei_hv'], nuclei_hv, nuclei_mask, self.device) +
                ...
                batch_sz = mif_img.shape[0]
                metrics['mif_nuclei_dice'] += compute_dice((outputs_mif['mif_nuclei_seg'] > 0.5).float(), nuclei_mask).item() * batch_sz
                metrics['mif_cell_dice'] += compute_dice((outputs_mif['mif_cell_seg'] > 0.5).float(), mif_cell_mask).item() * batch_sz
                metrics['mif_nuclei_hv_std'] += outputs_mif['mif_nuclei_hv'].std().item() * batch_sz
                metrics['mif_cell_hv_std'] += outputs_mif['mif_cell_hv'].std().item() * batch_sz
                
                val_mif_count += batch_sz
                val_batch_count += 1
            
            # Normalize Flex Metrics properly
            # Loss is approximate average over batch steps
            metrics['loss'] = val_loss_accum / (max(val_batch_count, 1) * 2)
            
            if val_he_count > 0:
                metrics['he_nuclei_dice'] /= val_he_count
                metrics['he_cell_dice'] /= val_he_count
