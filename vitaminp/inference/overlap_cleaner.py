...
class OverlapCleaner:
    ...
    def _remove_duplicates(self, detections: pd.DataFrame) -> pd.DataFrame:
        ...
        for iteration in range(self.max_iterations):
            # Create polygon list (now returns list of dicts)
            poly_dicts = self._create_polygon_list(cleaned)
            
            if len(poly_dicts) == 0:
                break
            
            # Extract just polygons for STRtree
            polygons = [pd['polygon'] for pd in poly_dicts]
            ...
            for i, query_dict in enumerate(poly_dicts):
                query_poly = query_dict['polygon']
                query_uid = query_dict['uid']
                
                if query_uid not in checked_indices:
                    ...
                    if len(intersected_indices) > 1:
                        ...
                        overlaps = []
                        for j in intersected_indices:
                            inter_dict = poly_dicts[j]
                            inter_poly = inter_dict['polygon']
                            inter_uid = inter_dict['uid']
                            
                            if inter_uid != query_uid and inter_uid not in checked_indices:
                                # Calculate overlap
                                intersection_area = query_poly.intersection(inter_poly).area
                                iou = intersection_area / min(query_poly.area, inter_poly.area)
                                
                                if iou > self.iou_threshold:
                                    overlaps.append(inter_dict)
                                    checked_indices.add(inter_uid)
                                    overlap_count += 1
                        
                        if len(overlaps) > 0:
                            # Select best polygon (largest area)
                            all_candidates = [query_dict] + overlaps
                            areas = [pd['polygon'].area for pd in all_candidates]
                            best_idx = np.argmax(areas)
                            keep_indices.append(all_candidates[best_idx]['uid'])
                        else:
                            keep_indices.append(query_uid)
                    else:
                        keep_indices.append(query_uid)
                    
                    checked_indices.add(query_uid)
    ...
    def _create_polygon_list(self, detections: pd.DataFrame) -> List[Dict]:
        ...
        Returns:
            List[Dict]: List of dicts with 'polygon' and 'uid' keys
        """
        poly_list = []
        ...
        for idx, row in detections.iterrows():
            ...
            # Store as dict instead of setting attribute
            poly_list.append({
                'polygon': poly,
                'uid': idx
            })
