...
Supports multiple cancer types (CRC, Xenium, TissueNet, PanNuke)
"""
...
class CRCZarrDataset(Dataset):
    ...
    def __init__(self, samples: List[str], config, training: bool = True):
        ...
        Args:
            samples: List of sample names (e.g., ['CRC01', 'breast', 'train', 'Fold1'])
    ...
    def _get_zarr_path(self, sample_name: str) -> str:
        ...
        # 2. PanNuke Folds (Fold1, Fold2, Fold3)
        ...
        # 4. Xenium Samples (Default fallback)
        else:
    ...
    def _build_patch_index(self):
            ...
            for sample in self.samples:
                ...
                # --- Case 1: TissueNet / PanNuke (Nested folder structure) ---
                if dataset_type in ['tissuenet', 'pannuke']:
                    # For TissueNet: sample='train'; path=.../tissuenet/train
                    # For PanNuke:   sample='Fold1'; path=.../pannuke/Fold1
                    ...
                    # Get all sub-directories (tissuenet_mosaic_X or pannuke_mosaic_X)
                    try:
                        sub_samples = sorted([
                        ...
                        for sub in sub_samples:
                            # Patch index is always 0 because your script saves 1 patch per Zarr folder
                            self.patch_info.append((sample, sub, 0))
                            
                        if self.config.verbose:
                            print(f"  ✅ {dataset_type} {sample}: {len(sub_samples)} samples loaded")
                    except Exception as e:
                        print(f"  ⚠️  {dataset_type} error reading {split_path}: {e}")

                # --- Case 2: CRC / Xenium (Standard multi-patch Zarr) ---
                else:
                    # Check for the 'he' folder first
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
            ...
            try:
                if dataset_type == 'tissuenet':
                    ...
                    # PanNuke Path: base/FoldX/pannuke_mosaic_X/
                    # NO 'mif' folder, files are at root
                    base_folder = os.path.join(zarr_path, sample_name, sub_sample)
                    
                    # Load H&E Data (Images + Nuclei Mask)
                    ...
                    h, w = he_img.shape[:2]
                    he_cell_mask = np.zeros((h, w), dtype=np.int32) # PanNuke has no cell masks
            ...
            # MIF: Using max value (65535.0 for uint16)
...
def create_dataloaders(config) -> Tuple[DataLoader, DataLoader, DataLoader]:
    ...
    if config.verbose:
        ...
        def count_types(samples):
            ...
            pn = sum(1 for s in samples if s.startswith('Fold'))
            tn = sum(1 for s in samples if s in ['train', 'val', 'test'])
            xen = len(samples) - crc - tn - pn
            return crc, xen, tn, pn

        c_tr, x_tr, t_tr, p_tr = count_types(train_samples)
        print(f"\nTrain split: {c_tr} CRC + {x_tr} Xenium + {t_tr} TissueNet + {p_tr} PanNuke")
        
        c_val, x_val, t_val, p_val = count_types(val_samples)
        print(f"Val split:   {c_val} CRC + {x_val} Xenium + {t_val} TissueNet + {p_val} PanNuke")
        
        c_te, x_te, t_te, p_te = count_types(test_samples)
        print(f"Test split:  {c_te} CRC + {x_te} Xenium + {t_te} TissueNet + {p_te} PanNuke")
        print(f"{'='*80}\n")
