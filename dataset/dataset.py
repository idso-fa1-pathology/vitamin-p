...
Supports multiple cancer types (CRC, Xenium, etc.)
"""
...
class CRCZarrDataset(Dataset):
    ...
    def __init__(self, samples: List[str], config, training: bool = True):
        ...
        Args:
            samples: List of sample names (e.g., ['CRC01', 'breast', 'cervical'])
    ...
    def _get_zarr_path(self, sample_name: str) -> str:
        ...
        Returns:
            Path to zarr base directory for this sample
        """
        # Determine which zarr base to use based on sample name
        if sample_name.startswith('CRC'):
            return self.zarr_bases['crc']
        else:
            # Assume Xenium samples (breast, cervical, etc.)
    ...
    def _build_patch_index(self):
        ...
        for sample in self.samples:
            # Get correct zarr path for this sample
            zarr_path = self._get_zarr_path(sample)
            he_path = f"{zarr_path}/{sample}/he/images.zarr"
            ...
            try:
                ...
                if self.config.verbose:
                    cancer_type = "CRC" if sample.startswith('CRC') else "Xenium"
                    print(f"  ✅ {sample} ({cancer_type}): {n_patches} patches")
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        he_img = zarr.open(f"{zarr_path}/{sample_name}/he/images.zarr", 'r')[patch_idx]
        he_nuclei_mask = zarr.open(f"{zarr_path}/{sample_name}/he/nuclei_masks.zarr", 'r')[patch_idx]
        he_cell_mask = zarr.open(f"{zarr_path}/{sample_name}/he/cell_masks.zarr", 'r')[patch_idx]
        
        # Load MIF data (2 channels)
        mif_img = zarr.open(f"{zarr_path}/{sample_name}/mif/images.zarr", 'r')[patch_idx]
        mif_nuclei_mask = zarr.open(f"{zarr_path}/{sample_name}/mif/nuclei_masks.zarr", 'r')[patch_idx]
        mif_cell_mask = zarr.open(f"{zarr_path}/{sample_name}/mif/cell_masks.zarr", 'r')[patch_idx]
...
def create_dataloaders(config) -> Tuple[DataLoader, DataLoader, DataLoader]:
    ...
    if config.verbose:
        ...
        crc_train = sum(1 for s in train_samples if s.startswith('CRC'))
        xenium_train = len(train_samples) - crc_train
        print(f"\nTrain split: {crc_train} CRC + {xenium_train} Xenium samples")
        
        crc_val = sum(1 for s in val_samples if s.startswith('CRC'))
        xenium_val = len(val_samples) - crc_val
        print(f"Val split: {crc_val} CRC + {xenium_val} Xenium samples")
        
        crc_test = sum(1 for s in test_samples if s.startswith('CRC'))
        xenium_test = len(test_samples) - crc_test
        print(f"Test split: {crc_test} CRC + {xenium_test} Xenium samples")
        print(f"{'='*80}\n")
