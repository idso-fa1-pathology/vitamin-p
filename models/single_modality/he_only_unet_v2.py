"""
HE-Only UNet V2 with flexible backbone support
Supports: ResNet, DINOv2, ConvNeXt, and more

This is backward compatible with the original he_only_unet.py
...
class HEOnlyUNetV2(nn.Module):
    """
    Single-encoder UNet for H&E pathology images only
    Now supports multiple backbone architectures!
    
    Architecture:
    - H&E (3ch): Backbone encoder layers 1-4 (full encoder)
    ...
    def __init__(self, backbone='resnet50', pretrained=True, dropout_rate=0.3):
        ...
        print(f"Building HE-only encoder with backbone: {backbone}")
        self.backbone, self.channels = BackboneBuilder.build(backbone, pretrained, in_channels=3)
        ...
        print(f"✓ HEOnlyUNetV2 initialized with {backbone} backbone")
    ...
    def _build_decoders(self):
        ...
        self.dec1_nuclei = self.conv_block(64 + channels[0], 64, dropout_rate * 0.5)  # ← Correct
        ...
        self.dec1_cell = self.conv_block(64 + channels[0], 64, dropout_rate * 0.5)  # ← Correct
    ...
    def forward(self, he_img):
        ...
        """
        # ========== ENCODER ==========
        features = self.backbone.extract_features(he_img)
        x0 = features['x0']
        x1 = features['x1']
        x2 = features['x2']
        x3 = features['x3']
        ...
        return {
            'he_nuclei_seg': nuclei_seg,
            'he_nuclei_hv': nuclei_hv,
            'he_cell_seg': cell_seg,
            'he_cell_hv': cell_hv,
        }