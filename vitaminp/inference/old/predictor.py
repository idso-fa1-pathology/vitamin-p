...
class WSIPredictor:
    ...
    def predict(
        ...
        Args:
            ...
            save_heatmap (bool): Save overlay visualization. Default: False
            save_json (bool): Save detections as JSON. Default: True
            save_csv (bool): Save detections as CSV. Default: False
            save_geojson (bool): Save detections as GeoJSON. Default: True
        ...
        for batch_idx, (pred_batch, meta_list) in enumerate(zip(predictions, tile_metadata)):
            # pred_batch is a dict with batched tensors: {key: [B, ...]}
            # meta_list should be a list of metadata dicts
            
            # ✅ DEBUG: Check structure
            if batch_idx == 0:
                self.logger.info(f"First batch - meta_list type: {type(meta_list)}")
                if isinstance(meta_list, list):
                    self.logger.info(f"  Meta list length: {len(meta_list)}")
                    if len(meta_list) > 0:
                        self.logger.info(f"  First meta keys: {meta_list[0].keys()}")
                self.logger.info(f"  Pred batch keys: {pred_batch.keys()}")
            ...
            if isinstance(meta_list, list):
                ...
                # meta_list is a single dict, which is wrong!
                self.logger.warning(f"meta_list is not a list! Type: {type(meta_list)}")
                batch_size = 1
                meta_list = [meta_list]  # Convert to list
            
            # Split the batch into individual predictions
            for i in range(batch_size):
                single_pred = {}
                for key, value in pred_batch.items():
                    # ✅ FIX: Properly extract based on tensor dimensions
                    if value.dim() == 4:  # [B, C, H, W] - standard case
                        single_pred[key] = value[i]  # -> [C, H, W]
                    elif value.dim() == 3:  # [B, H, W] - should not happen for HV, but handle it
                        # This indicates the model output is wrong!
                        self.logger.warning(
                            f"Unexpected 3D tensor for {key}: {value.shape}. "
                            f"Expected 4D [B, C, H, W]. Adding channel dimension."
                        )
                        single_pred[key] = value[i].unsqueeze(0)  # -> [1, H, W]
                    elif value.dim() == 2:  # [B, X] or [H, W]
                        # This is definitely wrong for image outputs
                        self.logger.error(
                            f"Invalid 2D tensor for {key}: {value.shape}. "
                            f"Cannot unbatch properly."
                        )
                        single_pred[key] = value[i] if value.shape[0] == batch_size else value
                    else:
                        # Unknown dimensionality, try simple indexing
                        single_pred[key] = value[i]
                
                flat_predictions.append(single_pred)
                flat_metadata.append(meta_list[i])

        self.logger.info(f"Unbatched: {len(flat_predictions)} predictions, {len(flat_metadata)} metadata")        
        ...
        if save_geojson:
            geojson_path = output_dir / f"{wsi_name}_{branch}_detections.geojson"
            save_detections_geojson(
                detections=all_detections,
            ...
            self.logger.info(f"Saved GeoJSON: {geojson_path}")
        
        # Save overlay visualization if requested
        if save_heatmap:
            self.logger.info("Generating overlay visualization...")
            
            overlay_path = output_dir / f"{wsi_name}_{branch}_overlay.png"
            create_overlay_visualization(
                wsi_path=wsi_path,
                detections=all_detections,
                output_path=overlay_path,
                downsample=8,  # Adjust based on image size
            ...
            self.logger.info(f"Saved overlay visualization: {overlay_path}")
